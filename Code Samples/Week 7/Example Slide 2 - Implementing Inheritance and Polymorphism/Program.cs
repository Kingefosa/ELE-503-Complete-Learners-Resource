...
{
    public HumiditySensor(string id) : base(id) { }
...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        daq.AddSensor(new HumiditySensor("HumiditySensor1"));

        while (true)
        {
            var data = daq.CollectData();
            Console.Clear();
            Console.WriteLine("Sensor Monitoring System");
            Console.WriteLine("------------------------");
            foreach (var entry in data)
            {
                Console.WriteLine($"{entry.Key}: {entry.Value:F2}" + (entry.Key.Contains("Temp") ? "°C" : "%"));
            }
